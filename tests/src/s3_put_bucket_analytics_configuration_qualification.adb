...
procedure S3_Put_Bucket_Analytics_Configuration_Qualification is
   ...
   use type Buckets.Bucket_Analytics_Configuration_Mutation_Disposition;
   use type Buckets.Put_Bucket_Analytics_Result_Kind;
   ...
   Document : constant String :=
     ...
     "</Id><StorageClassAnalysis></StorageClassAnalysis>" &
     "</AnalyticsConfiguration>";
   Value : constant Analytics.Analytics_Configuration :=
     Analytics.Parse (Document, Limits);
      ...
      if Expected = "success" then
         Require
           (Result.Kind = Buckets.Put_Bucket_Analytics_Response_Available
            and then Result.Disposition =
              Buckets.Bucket_Analytics_Configuration_Mutation_Completed
         ...
         Require
           (Result.Kind = Buckets.Put_Bucket_Analytics_Exchange_Failed
            and then Result.Disposition =
              Buckets.Bucket_Analytics_Configuration_Mutation_Outcome_Unknown
...
begin
   ...
   if Lane = "low_level" then
      ...
      Check_Result
        (Buckets.Set_Analytics_Configuration
      ...
      declare
         ...
         Operation : Buckets.Put_Bucket_Analytics_Operation :=
           Buckets.Set_Analytics_Configuration
         ...
         Result : Buckets.Put_Bucket_Analytics_Result;
      begin
   ...
   Ada.Text_IO.Put_Line
     ("PutBucketAnalyticsConfiguration signed qualification " & Case_ID &
      ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_Put_Bucket_Analytics_Configuration_Qualification;
