...
procedure S3_Get_Bucket_Metrics_Configuration_Qualification is
   ...
   use type Buckets.Get_Bucket_Metrics_Result_Kind;
   ...
   procedure Check_Result (Result : Buckets.Get_Bucket_Metrics_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind = Buckets.Get_Bucket_Metrics_Response_Available
         ...
         Require
           (Result.Kind = Buckets.Get_Bucket_Metrics_Exchange_Failed
...
begin
   ...
   if Lane = "low_level" then
      declare
         ...
         Result : constant
           Low_Level.Get_Bucket_Metrics_Configuration_Outcome :=
           Low_Level.Execute_Get_Bucket_Metrics_Configuration
      ...
      Check_Result
        (Buckets.Get_Metrics_Configuration
      ...
      declare
         ...
         Operation : Buckets.Get_Bucket_Metrics_Operation :=
           Buckets.Get_Metrics_Configuration
         ...
         Result : Buckets.Get_Bucket_Metrics_Result;
      begin
         ...
         Require
           (Expected = "not_found"
            and then Result.Kind =
              Buckets.Get_Bucket_Metrics_Response_Available
            and then Result.Failure = Client.Not_Found,
   ...
   Ada.Text_IO.Put_Line
     ("GetBucketMetricsConfiguration signed qualification " & Case_ID &
      ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_Get_Bucket_Metrics_Configuration_Qualification;
