...
package body Flyology.Object_Storage.Client.Objects.Testing is
   ...
   procedure Check_Retention_Certainty_Corpus is
      type Failure_Kind_Array is array (Positive range <>) of
        HTTP_Client.Exchange_Result_Kind;
      Failure_Kinds : constant Failure_Kind_Array :=
        (HTTP_Client.Pre_Admission_Rejected,
      ...
      function Get_Value
        (Status : Flyology.HTTP.Status_Code;
         Code   : String := "")
         return Low_Level.Get_Object_Retention_Outcome is
        (if Status = 200
         then (Kind => Low_Level.Object_Retention_Found,
               Status => Status, Retention => (others => <>))
         else (Kind => Low_Level.Get_Object_Retention_Rejected,
               Status => Status, Error => Error_Response (Code)));

      function Put_Value
        (Status : Flyology.HTTP.Status_Code;
         Code   : String := "")
         return Low_Level.Put_Object_Retention_Outcome is
        (if Status = 200
         then (Kind => Low_Level.Object_Retention_Updated,
               Status => Status, Result => (others => <>))
         else (Kind => Low_Level.Put_Object_Retention_Rejected,
               Status => Status, Error => Error_Response (Code)));
      ...
      is
         Result : constant Get_Retention_Result :=
           Normalize_Get_Retention_Response
             (Get_Value (Status, Code), HTTP_Client.Response_Observed);
      begin
         ...
         then
            raise Program_Error with
              "GetObjectRetention response normalization mismatch";
         end if;
      end Check_Get_Response;
      ...
      is
         Result : constant Put_Retention_Result :=
           Normalize_Put_Retention_Response
             (Put_Value (Status, Code), HTTP_Client.Response_Observed);
      begin
         ...
         then
            raise Program_Error with
              "PutObjectRetention response normalization mismatch";
         end if;
      end Check_Put_Response;
      ...
      procedure Check_Failure
        (Kind      : HTTP_Client.Exchange_Result_Kind;
         Admission : HTTP_Client.Admission_Certainty)
      is
         Get_Result : constant Get_Retention_Result :=
           Normalize_Get_Retention_Failure
             (Kind, Admission, HTTP_Client.Waiting_Response_Head);
         Put_Result : constant Put_Retention_Result :=
           Normalize_Put_Retention_Failure
             (Kind, Admission, HTTP_Client.Waiting_Response_Head);
      begin
         if Get_Result.Kind /= Get_Retention_Exchange_Failed
           or else Get_Result.Failure /= Expected_Failure (Kind)
           or else Get_Result.Admission /= Admission
           or else Get_Result.HTTP_Result /= Kind
           or else Put_Result.Kind /= Put_Retention_Exchange_Failed
           or else Put_Result.Disposition /=
             Expected_Disposition (Kind, Admission)
           or else Put_Result.Failure /= Expected_Failure (Kind)
           or else Put_Result.Admission /= Admission
         ...
         then
            raise Program_Error with
              "Object Retention exchange normalization mismatch";
         end if;
      end Check_Failure;
   begin
      Check_Get_Response (200, "", No_Failure);
      ...
      Check_Put_Response
        (200, "", Retention_Mutation_Completed, No_Failure);
      Check_Put_Response
        (400, "MalformedXML",
         Retention_Mutation_Definitely_Not_Applied, Invalid_Request);
      Check_Put_Response
        (403, "AccessDenied",
         Retention_Mutation_Definitely_Not_Applied, Authorization_Failed);
      Check_Put_Response
        (404, "NoSuchVersion",
         Retention_Mutation_Definitely_Not_Applied, Not_Found);
      Check_Put_Response
        (409, "OperationAborted", Retention_Mutation_Outcome_Unknown,
         Unavailable_Or_Retryable);
      Check_Put_Response
        (500, "InternalError", Retention_Mutation_Outcome_Unknown,
         Unavailable_Or_Retryable);
      Check_Put_Response
        (403, "", Retention_Mutation_Outcome_Unknown,
         Corrupt_Or_Invalid_Response);
      ...
      loop
         ...
         begin
            ...
            then
               raise Program_Error with
                 "inconsistent Object Retention certainty was accepted";
            end if;
         end;
      end loop;
   ...
   end Check_Retention_Certainty_Corpus;
   ...
   is
      ...
      Set : aliased Flyology.Operations.Completion_Set (3);
      Wrong : aliased Low_Level.Prepared_Request := Prepared;
      Get_Operation : aliased Get_Retention_Operation
        (Set'Access, Client, null);
      Put_Operation : aliased Put_Retention_Operation
        (Set'Access, Client, null);
      Get_Rejected : Boolean := False;
      Put_Rejected : Boolean := False;
   begin
      begin
         Low_Level.Get_Object_Retention
           (Client, Wrong'Access, Get_Operation'Access, Deadline, null,
            Get_Operation.Child);
      exception
         when Low_Level.Invalid_Request => Get_Rejected := True;
      ...
      begin
         Low_Level.Put_Object_Retention
           (Client, Wrong'Access, Put_Operation'Access, Put_Operation'Access,
            Deadline, null, Put_Operation.Child);
      exception
         when Low_Level.Invalid_Request => Put_Rejected := True;
      ...
      then
         raise Program_Error with
           "Object Retention wrong prepared operation crossed admission";
      end if;
   end Check_Retention_Pre_Admission_Rejection;
   ...
   procedure Check_Get_Object_ACL_Result_Corpus is
      ...
      procedure Check_Response
        (Status  : Flyology.HTTP.Status_Code;
         ...
         Failure : Failure_Reason)
      is
         Value : constant Low_Level.Get_Object_ACL_Outcome :=
           (if Status = 200
            then
              (Kind   => Low_Level.Object_ACL_Found,
               Status => Status,
               Result => (others => <>))
            else
              (Kind   => Low_Level.Get_Object_ACL_Rejected,
               Status => Status,
      ...
      begin
         ...
         then
            raise Program_Error with
              "GetObjectAcl response normalization mismatch";
         end if;
      end Check_Response;
   begin
      Check_Response (200, "", No_Failure);
      Check_Response (400, "InvalidArgument", Invalid_Request);
      Check_Response (400, "InvalidBucketName", Invalid_Request);
      Check_Response (400, "InvalidRequest", Invalid_Request);
      Check_Response (400, "InvalidVersionId", Invalid_Request);
      ...
      Check_Response (404, "NoSuchKey", Not_Found);
      Check_Response (404, "NoSuchVersion", Not_Found);
         ...
         declare
            Value : constant Low_Level.Get_Object_ACL_Outcome :=
              (Kind   => Low_Level.Object_ACL_Found,
               ...
               Result => (others => <>));
            Result : constant Get_Object_ACL_Result :=
              Normalize_Get_Object_ACL_Response (Value, Admission);
         begin
            if Result.Failure /= Corrupt_Or_Invalid_Response then
               raise Program_Error with
                 "inconsistent GetObjectAcl certainty was accepted";
            end if;
         end;
      end loop;
      ...
      for Kind of Failure_Kinds loop
         for Admission in HTTP_Client.Admission_Certainty loop
            ...
            begin
               ...
               then
                  raise Program_Error with
                    "GetObjectAcl exchange normalization mismatch";
               end if;
            end;
         end loop;
      end loop;
