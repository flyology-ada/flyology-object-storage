...
package body Flyology.Object_Storage.Client.Buckets.Testing is
   ...
   procedure Check_Put_Bucket_Replication_Result_Corpus is
      type Failure_Kind_Array is array (Positive range <>) of
        HTTP_Client.Exchange_Result_Kind;
      Failure_Kinds : constant Failure_Kind_Array :=
        (HTTP_Client.Pre_Admission_Rejected,
      ...
      procedure Check_Response
        (Status : Flyology.HTTP.Status_Code; Code : String;
         Expected : Bucket_Replication_Mutation_Disposition;
         Failure : Failure_Reason)
      is
      ...
      begin
         ...
         then
            raise Program_Error with
              "PutBucketReplication normalization mismatch";
         end if;
      end Check_Response;
   begin
      Check_Response
        (200, "", Bucket_Replication_Mutation_Completed, No_Failure);
      Check_Response
        (400, "MalformedXML",
      ...
      Check_Response
        (409, "OperationAborted",
         ...
         Unavailable_Or_Retryable);
      Check_Response
        (503, "SlowDown", Bucket_Replication_Mutation_Outcome_Unknown,
         Unavailable_Or_Retryable);
      Check_Response
        (409, "", Bucket_Replication_Mutation_Outcome_Unknown,
         Corrupt_Or_Invalid_Response);
      ...
      loop
         ...
         begin
            ...
            then
               raise Program_Error with
                 "inconsistent replication mutation certainty accepted";
            end if;
         end;
      end loop;
            ...
            declare
               Result : constant Put_Bucket_Replication_Result :=
                 Normalize_Put_Bucket_Replication_Failure
                   (Kind, Admission, HTTP_Client.Waiting_Response_Head, "");
            begin
               if Result.Kind /= Put_Bucket_Replication_Exchange_Failed
                 or else Result.Disposition /=
                   Expected_Disposition (Kind, Admission)
                 or else Result.Failure /= Expected_Failure (Kind)
                 or else Result.Admission /= Admission
               ...
               then
                  raise Program_Error with
                    "replication mutation exchange certainty mismatch";
               end if;
            end;
         end loop;
      end loop;
   end Check_Put_Bucket_Replication_Result_Corpus;
