...
procedure S3_List_Bucket_Analytics_Configurations_Qualification is
   ...
   use type Buckets.List_Bucket_Analytics_Result_Kind;
   use type HTTP_Client.Exchange_Result_Kind;
   use type Low_Level.List_Bucket_Analytics_Configurations_Outcome_Kind;
   ...
   procedure Check_Success_Page
     (Value : Low_Level.List_Bucket_Analytics_Configurations_Outcome) is
   begin
      Require
        (Value.Kind = Low_Level.Bucket_Analytics_Configurations_Listed,
      ...
      if Expected_Value'Length = 0 then
         ...
         Require
           (not Value.Result.Configurations.Is_Empty
            ...
            "first analytics identifier mismatch");
      end if;
      if Case_ID = "low-level-success" then
         ...
         Require
           (Value.Result.Has_Is_Truncated
            ...
            and then not Value.Result.Configurations.First_Element.
              Storage_Class_Analysis.Data_Export.Is_Set
            and then US.To_String
              (Value.Result.Configurations.Last_Element.ID) =
                "second-analytics",
            "synchronous page envelope or order mismatch");
   ...
   procedure Check_Result (Result : Buckets.List_Bucket_Analytics_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind = Buckets.List_Bucket_Analytics_Response_Available
         ...
         Require
           (Result.Kind = Buckets.List_Bucket_Analytics_Exchange_Failed
...
begin
   ...
   if Lane = "low_level" then
      declare
         Prepared : constant Low_Level.Prepared_Request :=
           Low_Level.Prepare_List_Bucket_Analytics_Configurations
             (Origin, Low_Level.Path_Style, Bucket, Parameters, Identity,
              Signed_Region, Signed_Timestamp);
         Result : constant
           Low_Level.List_Bucket_Analytics_Configurations_Outcome :=
           Low_Level.Execute_List_Bucket_Analytics_Configurations
      ...
      Check_Result
        (Buckets.List_Analytics_Configurations
      ...
      declare
         ...
         Operation : Buckets.List_Bucket_Analytics_Operation :=
           Buckets.List_Analytics_Configurations
         ...
         Result : Buckets.List_Bucket_Analytics_Result;
      begin
         ...
         Require
           (Expected = "not_found"
            and then Result.Kind =
              Buckets.List_Bucket_Analytics_Response_Available
            and then Result.Failure = Client.Not_Found,
   ...
   Ada.Text_IO.Put_Line
     ("ListBucketAnalyticsConfigurations signed qualification " & Case_ID &
      ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_List_Bucket_Analytics_Configurations_Qualification;
