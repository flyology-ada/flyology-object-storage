...
procedure S3_List_Bucket_Metrics_Configurations_Qualification is
   ...
   use type Buckets.List_Bucket_Metrics_Result_Kind;
   use type HTTP_Client.Exchange_Result_Kind;
   use type Low_Level.List_Bucket_Metrics_Configurations_Outcome_Kind;
   ...
   procedure Check_Success_Page
     (Value : Low_Level.List_Bucket_Metrics_Configurations_Outcome) is
   begin
      Require
        (Value.Kind = Low_Level.Bucket_Metrics_Configurations_Listed,
      ...
      if Expected_Value'Length = 0 then
         ...
         Require
           (not Value.Result.Configurations.Is_Empty
            ...
            "first metrics identifier mismatch");
      end if;
   ...
   procedure Check_Result (Result : Buckets.List_Bucket_Metrics_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind = Buckets.List_Bucket_Metrics_Response_Available
         ...
         Require
           (Result.Kind = Buckets.List_Bucket_Metrics_Exchange_Failed
...
begin
   ...
   if Lane = "low_level" then
      declare
         Prepared : constant Low_Level.Prepared_Request :=
           Low_Level.Prepare_List_Bucket_Metrics_Configurations
             (Origin, Low_Level.Path_Style, Bucket, Parameters, Identity,
              Signed_Region, Signed_Timestamp);
         Result : constant
           Low_Level.List_Bucket_Metrics_Configurations_Outcome :=
           Low_Level.Execute_List_Bucket_Metrics_Configurations
      ...
      Check_Result
        (Buckets.List_Metrics_Configurations
      ...
      declare
         ...
         Operation : Buckets.List_Bucket_Metrics_Operation :=
           Buckets.List_Metrics_Configurations
         ...
         Result : Buckets.List_Bucket_Metrics_Result;
      begin
         ...
         Require
           (Expected = "not_found"
            and then Result.Kind =
              Buckets.List_Bucket_Metrics_Response_Available
            and then Result.Failure = Client.Not_Found,
   ...
   Ada.Text_IO.Put_Line
     ("ListBucketMetricsConfigurations signed qualification " & Case_ID &
      ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_List_Bucket_Metrics_Configurations_Qualification;
