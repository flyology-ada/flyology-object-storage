...
procedure S3_List_Bucket_Intelligent_Tiering_Configurations_Qualification is
   ...
   use type Buckets.List_Bucket_Intelligent_Tiering_Result_Kind;
   use type HTTP_Client.Exchange_Result_Kind;
   use type Intelligent_Tiering.Configuration_Status;
   use type Intelligent_Tiering.Access_Tier_Kind;
   use type
     Low_Level.List_Bucket_Intelligent_Tiering_Configurations_Outcome_Kind;
   ...
   procedure Check_Success_Page
     (Value :
        Low_Level.List_Bucket_Intelligent_Tiering_Configurations_Outcome) is
   begin
      Require
        (Value.Kind =
           Low_Level.Bucket_Intelligent_Tiering_Configurations_Listed,
      ...
      if Expected_Value'Length = 0 then
         ...
         Require
           (not Value.Result.Configurations.Is_Empty
            ...
            "first Intelligent-Tiering identifier mismatch");
      end if;
      if Case_ID = "low-level-success" then
         ...
         Require
           (Value.Result.Has_Is_Truncated
            ...
            and then Value.Result.Configurations.First_Element.Filter.
              And_Predicates.Is_Set
            and then Value.Result.Configurations.First_Element.Filter.
              And_Predicates.Prefix.Is_Set
            and then US.To_String
              (Value.Result.Configurations.First_Element.Filter.
                 And_Predicates.Prefix.Value) = "logs/"
            and then Value.Result.Configurations.First_Element.Filter.
              And_Predicates.Tags.Length = 1
            and then US.To_String
              (Value.Result.Configurations.First_Element.Filter.
                 And_Predicates.Tags.First_Element.Key) = "team"
            and then US.To_String
              (Value.Result.Configurations.First_Element.Filter.
                 And_Predicates.Tags.First_Element.Value) = "storage"
            and then Value.Result.Configurations.First_Element.Status =
              Intelligent_Tiering.Enabled
            and then Value.Result.Configurations.First_Element.Tierings.Length
              = 2
            and then Value.Result.Configurations.First_Element.Tierings.
              Last_Element.Access_Tier =
                Intelligent_Tiering.Deep_Archive_Access
            and then US.To_String
              (Value.Result.Configurations.Last_Element.ID) = "second-tiering"
            and then Value.Result.Configurations.Last_Element.Status =
              Intelligent_Tiering.Disabled,
   ...
   procedure Check_Result
     (Result : Buckets.List_Bucket_Intelligent_Tiering_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind =
              Buckets.List_Bucket_Intelligent_Tiering_Response_Available
         ...
         Require
           (Result.Kind =
              Buckets.List_Bucket_Intelligent_Tiering_Exchange_Failed
...
begin
   ...
   if Lane = "low_level" then
      declare
         Prepared : constant Low_Level.Prepared_Request :=
           Low_Level.Prepare_List_Bucket_Intelligent_Tiering_Configurations
         ...
         Result : constant
           ...
           Low_Level.Execute_List_Bucket_Intelligent_Tiering_Configurations
      ...
      Check_Result
        (Buckets.List_Intelligent_Tiering_Configurations
      ...
      declare
         ...
         Operation : Buckets.List_Bucket_Intelligent_Tiering_Operation :=
           Buckets.List_Intelligent_Tiering_Configurations
         ...
         Result : Buckets.List_Bucket_Intelligent_Tiering_Result;
      begin
   ...
   Ada.Text_IO.Put_Line
     ("ListBucketIntelligentTieringConfigurations signed qualification " &
      Case_ID & ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_List_Bucket_Intelligent_Tiering_Configurations_Qualification;
