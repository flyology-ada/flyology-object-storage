...
procedure S3_Get_Bucket_Replication_Qualification is
   ...
   use type Buckets.Get_Bucket_Replication_Result_Kind;
   ...
   Identity : constant Low_Level.Credentials :=
     --  AWS SigV4 published-example identity used by the existing signed
     --  socket corpus; changing it invalidates the request-signing oracle.
   ...
   --  Five seconds is the repository's existing loopback socket-corpus
   --  watchdog. It bounds only this executable test, not a client default.
   ...
   --  Every case is serial. One HTTP slot is therefore the derived minimum
   ...
   procedure Check_Result (Result : Buckets.Get_Bucket_Replication_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind =
              Buckets.Get_Bucket_Replication_Response_Available
            and then Result.Failure = Client.No_Failure
            and then US.To_String (Result.Response.Configuration.Role) =
              Expected_Value,
         ...
         Require
           (Result.Kind = Buckets.Get_Bucket_Replication_Exchange_Failed
            and then Result.Failure =
              Client.Corrupt_Or_Invalid_Response
...
begin
   ...
   if Lane = "low_level" then
      ...
      begin
         Require
           (Expected = "success"
            ...
            and then US.To_String (Result.Configuration.Role) =
              Expected_Value,
            "low-level result mismatch");
      end;
   elsif Lane = "synchronous" or else Lane = "invalid_xml" then
      Check_Result
        (Buckets.Get_Replication_Configuration
      ...
      declare
         --  Derived capacity: provider parent, HTTP exchange, and HTTP's one
         --  active transport child are the only simultaneous operations.
         Set : aliased Operations.Completion_Set (3);
         Operation : Buckets.Get_Bucket_Replication_Operation :=
           Buckets.Get_Replication_Configuration
         ...
         Result : Buckets.Get_Bucket_Replication_Result;
      begin
      ...
      declare
         --  Same derived three-item owner stack as the composable lane.
         Set : aliased Operations.Completion_Set (3);
         Operation : Buckets.Get_Bucket_Replication_Operation :=
           Buckets.Get_Replication_Configuration
             (Set'Access, HTTP'Access, Origin, Bucket, Parameters, Identity,
   ...
   Ada.Text_IO.Put_Line
     ("GetBucketReplication signed qualification " & Case_ID & ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_Get_Bucket_Replication_Qualification;
