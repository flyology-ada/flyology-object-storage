...
procedure S3_Get_Bucket_Inventory_Configuration_Qualification is
   ...
   use type Buckets.Get_Bucket_Inventory_Result_Kind;
   ...
   procedure Check_Result
     (Result : Buckets.Get_Bucket_Inventory_Result) is
   begin
      if Expected = "success" then
         Require
           (Result.Kind =
              Buckets.Get_Bucket_Inventory_Response_Available
            and then Result.Failure = Client.No_Failure
            and then US.To_String (Result.Response.Configuration.ID) =
              Expected_Value,
            "typed success mismatch");
         if Expected_Value = "synchronous-inventory" then
            declare
               Configuration : Inventory.Inventory_Configuration renames
                 Result.Response.Configuration;
               Destination : Inventory.S3_Bucket_Destination renames
                 Configuration.Destination.S3_Bucket;
            begin
               Require
                 (Configuration.Is_Enabled
                  and then Configuration.Filter.Is_Set
                  and then US.To_String (Configuration.Filter.Prefix) =
                    "logs/"
                  and then Configuration.Versions = Inventory.All_Versions
                  and then Natural (Configuration.Optional_Fields.Length) = 3
                  and then Configuration.Optional_Fields.Element (1) =
                    Inventory.ETag
                  and then Configuration.Optional_Fields.Element (2) =
                    Inventory.Checksum_Algorithm
                  and then Configuration.Optional_Fields.Element (3) =
                    Inventory.Lifecycle_Expiration_Date
                  and then Configuration.Schedule.Frequency = Inventory.Weekly
                  and then Destination.Account_ID.Is_Set
                  and then US.To_String (Destination.Account_ID.Value) =
                    "123456789012"
                  and then US.To_String (Destination.Bucket) =
                    "arn:aws:s3:::qualified-destination"
                  and then Destination.Format = Inventory.Parquet
                  and then Destination.Prefix.Is_Set
                  and then US.To_String (Destination.Prefix.Value) =
                    "inventory/"
                  and then Destination.Encryption.Is_Set
                  and then Destination.Encryption.SSE_S3
                  and then Destination.Encryption.SSE_KMS_Key_ID.Is_Set
                  and then
                    US.To_String
                      (Destination.Encryption.SSE_KMS_Key_ID.Value) =
                    "key-123",
                  "complete typed inventory graph mismatch");
            end;
         end if;
         ...
         Require
           (Result.Kind =
              Buckets.Get_Bucket_Inventory_Exchange_Failed
...
begin
   ...
   if Lane = "low_level" then
      declare
         Prepared : constant Low_Level.Prepared_Request :=
           Low_Level.Prepare_Get_Bucket_Inventory_Configuration
         ...
         Result : constant
           ...
           Low_Level.Execute_Get_Bucket_Inventory_Configuration
      ...
      Check_Result
        (Buckets.Get_Inventory_Configuration
      ...
      declare
         ...
         Operation : Buckets.Get_Bucket_Inventory_Operation :=
           Buckets.Get_Inventory_Configuration
         ...
         Result : Buckets.Get_Bucket_Inventory_Result;
      begin
   ...
   Ada.Text_IO.Put_Line
     ("GetBucketInventoryConfiguration signed qualification " &
      Case_ID & ": OK");
exception
   when others =>
      HTTP_Client.Shutdown (HTTP);
      raise;
end S3_Get_Bucket_Inventory_Configuration_Qualification;
