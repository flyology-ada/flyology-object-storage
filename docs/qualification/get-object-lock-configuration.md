...
synchronous clients plus corpus for `GetObjectLockConfiguration`. Shared
Flyology backend persistence and authenticated server-route evidence is
recorded in `object-lock-server.md`; external-server interoperability is not
claimed.
...
The machine ledger records `GetObjectLockConfiguration` as `covered / covered
/ covered / covered`. This client record remains operation-specific; the
shared Object Lock record owns backend persistence, authenticated routing, and
