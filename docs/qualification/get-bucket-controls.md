...
`GetBucketAccelerateConfiguration`, `GetBucketPolicy`,
...
It records the five exact GET operations, request and output shapes, subresource
URIs, 200 success codes, all 22 request/output/nested members, member shapes,
wire locations, required flags, and the four relevant enum domains.
...
Accelerate status preserves absent, `Enabled`, and `Suspended`. Request payment
...
The four structured outputs use the strict shallow S3 XML parser. They accept
...
inventory verifier reported five operations, 22 request/output/nested members,
four exact enum domains, and thirteen reciprocal vectors; the 116-operation
