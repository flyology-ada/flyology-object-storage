...
package body Flyology.Object_Storage.Client.REST_XML_Mutations is
   ...
   function Declared_Length
     (Item : State) return HTTP_Client.Body_Length is
   ...
   procedure Release_Source (Item : in out State) is
   begin
   ...
   begin
      ...
      if HTTP_Client.Kind (HTTP_Result) /= HTTP_Client.Response_Complete then
         ...
         begin
            Item.Final_Result :=
              Decode_Response
                (HTTP_Client.Status (Response), Response,
   ...
   procedure Drive
     (Item         : in out State;
      ...
      Source       : not null access
        HTTP_Client.Operation_Request_Body_Source'Class;
   ...
   begin
      if Event = Operations.Start_Operation then
         Start_Exchange
           (Client, Item.Prepared'Access, Source, Sink, Item.Deadline,
