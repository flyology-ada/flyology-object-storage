...
package body Flyology.Object_Storage.Backends.Files is
   ...
   Bucket_Policy_Magic : constant String := "FOSPOL01";
   --  Persisted-format discriminator for one length-prefixed raw bucket
   --  policy. Changing it would make existing policy files unreadable.
   ...
   --  Persisted files-layout name paired with FOSPOL01. Renaming it would
   --  hide an existing bucket policy from later readers.
   function Bucket_Policy_Path
     (Item : Store; Bucket : String) return String is
     (Join (Configuration_Path (Item, Bucket), "policy.fos"));

   --  Persisted files-layout name paired with FOSCOR01. Renaming it would
   --  hide an existing bucket CORS configuration from later readers.
   ...
   function Bucket_Analytics_Path
     ...
     (Join
        (Configuration_Path (Item, Bucket),
         "analytics-" & GNAT.SHA256.Digest (Identifier) & ".fos"));

   function Bucket_Metrics_Path
     (Item : Store; Bucket, Identifier : String) return String is
     (Join
        (Configuration_Path (Item, Bucket),
   ...
   function Bucket_ABAC_Path
     (Item : Store; Bucket : String) return String is
     (Join (Configuration_Path (Item, Bucket), "abac.fos"));

   function Bucket_Acceleration_Path
     (Item : Store; Bucket : String) return String is
     (Join (Configuration_Path (Item, Bucket), "acceleration.fos"));
   ...
   File_String_IO_Chunk_Bytes : constant Positive := 64 * 1_024;
   --  Match the files backend's existing streaming chunk convention while
   --  bounding the stack working set independently of the admitted value.

   procedure Write_String (File : in out SIO.File_Type; Value : String) is
      Data : Ada.Streams.Stream_Element_Array
        (1 .. Ada.Streams.Stream_Element_Offset
          (File_String_IO_Chunk_Bytes));
      Source_Position : Integer := Value'First;
   begin
      while Source_Position <= Value'Last loop
         declare
            Count : constant Natural :=
              Natural'Min
                (File_String_IO_Chunk_Bytes,
                 Value'Last - Source_Position + 1);
            ...
            for Offset in 0 .. Count - 1 loop
               Data
                 ...
                 Ada.Streams.Stream_Element
                   (Character'Pos (Value (Source_Position + Offset)));
            end loop;
            Write_Bytes (File, Data (Data'First .. Last));
   ...
   is
      ...
      Data   : Ada.Streams.Stream_Element_Array
        (1 .. Ada.Streams.Stream_Element_Offset
          (File_String_IO_Chunk_Bytes));
      Target_Position : Positive := Result'First;
      Remaining       : Natural := Length;
   begin
      while Remaining > 0 loop
         declare
            Count : constant Natural :=
              Natural'Min (File_String_IO_Chunk_Bytes, Remaining);
            Last : constant Ada.Streams.Stream_Element_Offset :=
              Data'First + Ada.Streams.Stream_Element_Offset (Count) - 1;
         begin
            Read_Exact (File, Data (Data'First .. Last));
            for Offset in 0 .. Count - 1 loop
               Result (Target_Position + Offset) := Character'Val
                 (Data
                    (Data'First
                     + Ada.Streams.Stream_Element_Offset (Offset)));
            end loop;
      ...
      while Remaining > 0 loop
         declare
            Count : constant Natural :=
              Natural'Min (File_String_IO_Chunk_Bytes, Remaining);
         begin
            US.Append (Value, Read_String (File, Count));
            Remaining := Remaining - Count;
         end;
      end loop;
   exception
   ...
   function Matches
     (Value : US.Unbounded_String; Expected : String) return Boolean
   is
   begin
      if US.Length (Value) /= Expected'Length then
         return False;
      end if;
   ...
   procedure Write_Bucket_Policy
     (File : in out SIO.File_Type; Policy : String) is
   begin
      if not Valid_Bucket_Policy (Policy) then
         raise Constraint_Error;
      end if;
      Write_String (File, Bucket_Policy_Magic);
      Write_U32 (File, Policy'Length);
      Write_String (File, Policy);
   end Write_Bucket_Policy;

   procedure Read_Bucket_Policy
     (File : in out SIO.File_Type; Policy : out US.Unbounded_String)
   is
      File_Magic : constant String :=
        Read_String (File, Bucket_Policy_Magic'Length);
      Length : constant Natural := Read_U32 (File);
   begin
      Policy := US.Null_Unbounded_String;
      if File_Magic /= Bucket_Policy_Magic
        or else Byte_Count (Length) > Maximum_Bucket_Policy_Bytes
        or else SIO.Size (File) /=
          SIO.Count (Bucket_Policy_Magic'Length + 4 + Length)
      then
         raise Ada.IO_Exceptions.Data_Error;
      end if;
      Policy := US.To_Unbounded_String (Read_String (File, Length));
   end Read_Bucket_Policy;

   procedure Write_Bucket_CORS
     (File : in out SIO.File_Type; Document : String) is
   begin
      if not Valid_Bucket_CORS_Document (Document) then
         raise Constraint_Error;
      end if;
      Write_String (File, Bucket_CORS_Magic);
      Write_U32 (File, Document'Length);
      Write_String (File, Document);
   end Write_Bucket_CORS;

   procedure Read_Bucket_CORS
     (File : in out SIO.File_Type; Document : out US.Unbounded_String)
   is
      File_Magic : constant String :=
        Read_String (File, Bucket_CORS_Magic'Length);
      Length : constant Natural := Read_U32 (File);
   begin
      Document := US.Null_Unbounded_String;
      if File_Magic /= Bucket_CORS_Magic
        or else Byte_Count (Length) > Maximum_Bucket_CORS_Bytes
        or else SIO.Size (File) /=
          SIO.Count (Bucket_CORS_Magic'Length + 4 + Length)
      then
         raise Ada.IO_Exceptions.Data_Error;
      end if;
   ...
   begin
      ...
      Write_U32 (File, Identifier'Length);
      Write_U32 (File, Document'Length);
      Write_String (File, Identifier);
      Write_String (File, Document);
   end Write_Named_Bucket_Configuration;

   procedure Read_Named_Bucket_Configuration
     (File       : in out SIO.File_Type;
      Magic      : String;
      Document   : out US.Unbounded_String;
      Identifier : out US.Unbounded_String)
   is
      File_Magic : constant String := Read_String (File, Magic'Length);
      Identifier_Length : constant Natural := Read_U32 (File);
      Document_Length : constant Natural := Read_U32 (File);
   begin
      Document := US.Null_Unbounded_String;
      Identifier := US.Null_Unbounded_String;
      if File_Magic /= Magic
        or else Byte_Count (Identifier_Length) >
          Maximum_Bucket_Configuration_Bytes
        or else Byte_Count (Document_Length) >
          Maximum_Bucket_Configuration_Bytes -
            Byte_Count (Identifier_Length)
        or else SIO.Size (File) /=
          SIO.Count
            (Magic'Length + 8 + Identifier_Length + Document_Length)
      then
         raise Ada.IO_Exceptions.Data_Error;
      end if;
      Read_Unbounded_String (File, Identifier_Length, Identifier);
      Read_Unbounded_String (File, Document_Length, Document);
   end Read_Named_Bucket_Configuration;

   procedure Write_Bucket_Analytics
     (File : in out SIO.File_Type; Document : String; Metadata : String) is
   begin
      Write_Named_Bucket_Configuration
        (File, Bucket_Analytics_Magic, Document, Metadata);
   end Write_Bucket_Analytics;

   procedure Read_Bucket_Analytics
     (File     : in out SIO.File_Type;
      Document : out US.Unbounded_String;
   ...
   begin
      Read_Named_Bucket_Configuration
        (File, Bucket_Analytics_Magic, Document, Metadata);
   end Read_Bucket_Analytics;

   procedure Write_Bucket_Metrics
     (File : in out SIO.File_Type; Document : String; Metadata : String) is
   begin
      Write_Named_Bucket_Configuration
        (File, Bucket_Metrics_Magic, Document, Metadata);
   end Write_Bucket_Metrics;

   procedure Read_Bucket_Metrics
     (File     : in out SIO.File_Type;
      Document : out US.Unbounded_String;
      Metadata : out US.Unbounded_String) is
   begin
   ...
   is
      ...
      Number    : Long_Long_Integer;
      Renamed   : Boolean := False;
      Target    : constant String := Bucket_CORS_Path (Item, Bucket);
   begin
      Check_Context (Token, Deadline);
      if not Valid_Bucket_Name (Bucket) then
         Result := Invalid_Request;
      ...
      Temp := US.To_Unbounded_String
        (Join
           (Temp_Path (Item),
            "bucket-cors-" & GNAT.SHA256.Digest
              (Bucket & Long_Long_Integer'Image (Number) &
               Ada.Calendar.Time'Image (Ada.Calendar.Clock))));
      Validate_New_Temp_Target (Item, US.To_String (Temp));
      SIO.Create (File, SIO.Out_File, US.To_String (Temp));
      Opened := True;
      Write_Bucket_CORS (File, Document);
      SIO.Close (File);
      Opened := False;
      Sync_File (Item, US.To_String (Temp));
   ...
   is
      ...
      Opened : Boolean := False;
      Locked : Boolean := False;
      Path   : constant String := Bucket_CORS_Path (Item, Bucket);
   begin
      Document := US.Null_Unbounded_String;
      Configured := False;
      Check_Context (Token, Deadline);
      ...
      then
         ...
         SIO.Open (File, SIO.In_File, Path);
         Opened := True;
         Read_Bucket_CORS (File, Document);
         SIO.Close (File);
         Opened := False;
         Configured := True;
         Result := Success;
   ...
   function Count_Named_Bucket_Configurations
     (Item : Store; Bucket, Prefix : String) return Natural
   is
      Search        : Ada.Directories.Search_Type;
   ...
   begin
      ...
      while Ada.Directories.More_Entries (Search) loop
         ...
         if GNAT.OS_Lib.Is_Symbolic_Link
           ...
           or else Ada.Directories.Kind (Current_Entry) /=
             Ada.Directories.Ordinary_File
         then
            raise Ada.IO_Exceptions.Data_Error;
         end if;
   ...
   exception
      when others =>
         ...
         raise;
   end Count_Named_Bucket_Configurations;
   ...
   procedure Put_Bucket_Configuration_Document
     (Item        : in out Store;
      ...
      Read        : Read_Bucket_Document_Access;
   ...
   begin
      ...
      if Ada.Directories.Exists (Target) and then Read /= null then
         declare
            Existing_Document : US.Unbounded_String;
            Existing_Metadata : US.Unbounded_String;
         begin
            SIO.Open (File, SIO.In_File, Target);
            Opened := True;
            Read (File, Existing_Document, Existing_Metadata);
            SIO.Close (File);
            Opened := False;
            if not Matches (Existing_Metadata, Metadata) then
               raise Ada.IO_Exceptions.Data_Error;
            end if;
         end;
      elsif not Ada.Directories.Exists (Target)
        and then Count_Prefix'Length > 0
        and then Count_Named_Bucket_Configurations
          (Item, Bucket, Count_Prefix) >=
            Maximum_Bucket_Named_Configurations
      then
         Item.Publication.Release;
         Locked := False;
         Ada.Directories.Delete_File (US.To_String (Temp));
         ...
         then
            ...
            if Read /= null then
               declare
                  ...
                  Stored_Identifier : US.Unbounded_String;
               begin
                  SIO.Open (File, SIO.In_File, Target);
                  Read (File, Document, Stored_Identifier);
                  SIO.Close (File);
                  if not Matches (Stored_Identifier, Identifier) then
                     raise Ada.IO_Exceptions.Data_Error;
                  end if;
               end;
            end if;
   ...
   begin
      Put_Bucket_Configuration_Document
        (Item, Bucket, Document, "", Token, Deadline, "bucket-encryption-",
         ...
         null, Valid_Bucket_Encryption_Document'Access, True, "", Result);
   ...
   begin
      Delete_Bucket_Configuration_Document
        (Item, Bucket, Token, Deadline,
         Bucket_Encryption_Path (Item, Bucket), null, "", Result);
   ...
   begin
      Put_Bucket_Configuration_Document
        (Item, Bucket, Document, "", Token, Deadline,
         ...
         null, Valid_Bucket_Ownership_Controls_Document'Access,
         True, "", Result);
   ...
   begin
      Delete_Bucket_Configuration_Document
        (Item, Bucket, Token, Deadline,
         Bucket_Ownership_Controls_Path (Item, Bucket), null, "", Result);
   ...
   begin
      Delete_Bucket_Configuration_Document
        (Item, Bucket, Token, Deadline,
         Bucket_Lifecycle_Path (Item, Bucket), null, "", Result);
   ...
   procedure Put_Named_Bucket_Configuration
     (Item       : in out Store;
      ...
      Token      : access Flyology.Cancellation.Token;
      Deadline   : Ada.Real_Time.Time;
      Temp_Prefix, Count_Prefix, Target : String;
      Write      : not null Write_Bucket_Document_Access;
      Read       : not null Read_Bucket_Document_Access;
      Result     : out Status) is
   begin
      ...
      then
         ...
         Put_Bucket_Configuration_Document
           (Item, Bucket, Document, Identifier, Token, Deadline, Temp_Prefix,
            Target, Write, Read, Valid_Bucket_Logging_Document'Access,
            Valid_Bucket_Named_Configuration (Identifier, Document),
            Count_Prefix, Result);
      end if;
   ...
   begin
      ...
      then
         ...
         Get_Bucket_Configuration_Document
           (Item, Bucket, Token, Deadline, Target, Read, Document,
            Stored_Identifier, Configured, Result);
         if Result = Success and then Configured
           and then not Matches (Stored_Identifier, Identifier)
         then
            Document := US.Null_Unbounded_String;
            Configured := False;
            Result := Backend_Unavailable;
   ...
   begin
      ...
      then
         ...
         Delete_Bucket_Configuration_Document
           (Item, Bucket, Token, Deadline, Target, Read, Identifier, Result);
      end if;
   ...
   begin
      Put_Named_Bucket_Configuration
        (Item, Bucket, Identifier, Document, Token, Deadline,
         ...
         Bucket_Analytics_Path (Item, Bucket, Identifier),
         Write_Bucket_Analytics'Access, Read_Bucket_Analytics'Access,
         Result);
   ...
   begin
      Get_Named_Bucket_Configuration
        (Item, Bucket, Identifier, Token, Deadline,
         Bucket_Analytics_Path (Item, Bucket, Identifier),
         Read_Bucket_Analytics'Access, Document, Configured, Result);
   ...
   begin
      Put_Named_Bucket_Configuration
        (Item, Bucket, Identifier, Document, Token, Deadline,
         ...
         Write_Bucket_Metrics'Access, Read_Bucket_Metrics'Access, Result);
   ...
   begin
      Get_Named_Bucket_Configuration
        (Item, Bucket, Identifier, Token, Deadline,
         Bucket_Metrics_Path (Item, Bucket, Identifier),
         Read_Bucket_Metrics'Access, Document, Configured, Result);
   ...
   is
      ...
      Number    : Long_Long_Integer;
      Renamed   : Boolean := False;
      Target    : constant String := Bucket_Policy_Path (Item, Bucket);
   begin
      Check_Context (Token, Deadline);
      if not Valid_Bucket_Name (Bucket) then
         Result := Invalid_Request;
      ...
      Temp := US.To_Unbounded_String
        (Join
           (Temp_Path (Item),
            "bucket-policy-" & GNAT.SHA256.Digest
              (Bucket & Long_Long_Integer'Image (Number) &
               Ada.Calendar.Time'Image (Ada.Calendar.Clock))));
      Validate_New_Temp_Target (Item, US.To_String (Temp));
      SIO.Create (File, SIO.Out_File, US.To_String (Temp));
      Opened := True;
      Write_Bucket_Policy (File, Policy);
      SIO.Close (File);
      Opened := False;
      Sync_File (Item, US.To_String (Temp));
   ...
   is
      ...
      Opened : Boolean := False;
      Locked : Boolean := False;
      Path   : constant String := Bucket_Policy_Path (Item, Bucket);
   begin
      Policy := US.Null_Unbounded_String;
      Configured := False;
      Check_Context (Token, Deadline);
      ...
      then
         ...
         SIO.Open (File, SIO.In_File, Path);
         Opened := True;
         Read_Bucket_Policy (File, Policy);
         SIO.Close (File);
         Opened := False;
         Configured := True;
         Result := Success;
   ...
   begin
      ...
      then
         ...
         then
            ...
            SIO.Open (File, SIO.In_File, Target);
            Opened := True;
            if SIO.Size (File) /= SIO.Count (Magic'Length + 1)
              or else Read_String (File, Magic'Length) /= Magic
            then
               raise Ada.IO_Exceptions.Data_Error;
            end if;
            declare
               Encoded : constant String := Read_String (File, 1);
            begin
               Value := Encoded (Encoded'First);
            end;
            SIO.Close (File);
            Opened := False;
            Result := Success;
   ...
   begin
      Put_Bucket_Control
        (Item, Bucket, Bucket_ABAC_Path (Item, Bucket), "abac",
         Bucket_ABAC_Magic,
         (case Value is
             when Bucket_ABAC_Disabled     => 'D',
             when Bucket_ABAC_Enabled      => 'E',
             when Bucket_ABAC_Unconfigured => 'U'),
   ...
   begin
      Get_Bucket_Control
        (Item, Bucket, Bucket_ABAC_Path (Item, Bucket), Bucket_ABAC_Magic,
         'D', Token, Deadline, Encoded, Result);
      if Result = Success then
         Value :=
           (case Encoded is
               when 'D' => Bucket_ABAC_Disabled,
               when 'E' => Bucket_ABAC_Enabled,
               when 'U' => Bucket_ABAC_Unconfigured,
               when others => raise Ada.IO_Exceptions.Data_Error);
      else
   ...
   overriding procedure Put_Bucket_Acceleration
     (Item     : in out Store;
      ...
      Result   : out Status)
   is
      Encoded : constant Character :=
        (case Value is
            when Bucket_Acceleration_Unconfigured => 'U',
            when Bucket_Acceleration_Enabled      => 'E',
            when Bucket_Acceleration_Suspended    => 'S');
   begin
      Put_Bucket_Control
        (Item, Bucket, Bucket_Acceleration_Path (Item, Bucket),
         "acceleration", Bucket_Acceleration_Magic, Encoded,
         Token, Deadline, Result);
   ...
   begin
      Get_Bucket_Control
        (Item, Bucket, Bucket_Acceleration_Path (Item, Bucket),
         Bucket_Acceleration_Magic, 'U', Token, Deadline, Encoded, Result);
      if Result = Success then
         Value :=
           (case Encoded is
               when 'U' => Bucket_Acceleration_Unconfigured,
               when 'E' => Bucket_Acceleration_Enabled,
               when 'S' => Bucket_Acceleration_Suspended,
               when others => raise Ada.IO_Exceptions.Data_Error);
      else
   ...
   begin
      Put_Bucket_Control
        (Item, Bucket, Bucket_Request_Payment_Path (Item, Bucket),
         "request-payment", Bucket_Request_Payment_Magic,
         (if Value = Requester_Pays then 'R' else 'O'),
         Token, Deadline, Result);
   ...
   begin
      Get_Bucket_Control
        (Item, Bucket, Bucket_Request_Payment_Path (Item, Bucket),
         Bucket_Request_Payment_Magic, 'O', Token, Deadline, Encoded, Result);
      if Result = Success then
         Value :=
           (case Encoded is
               when 'O' => Bucket_Owner_Pays,
               when 'R' => Requester_Pays,
               when others => raise Ada.IO_Exceptions.Data_Error);
      else
