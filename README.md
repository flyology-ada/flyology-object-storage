...
- completion-set-aware conditional Put, bounded whole Get, generation-bound
  ...
  HeadBucket, bounded GetBucketLocation, and bounded non-replaying
  Put/GetBucketVersioning, with typed synchronous overloads that wait on the
  same owner-driven state machines and never create a per-operation helper
  task;
