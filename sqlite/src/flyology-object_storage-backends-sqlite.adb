...
package body Flyology.Object_Storage.Backends.SQLite is
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_Tags (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Put_Bucket_CORS (Item.Catalog, Bucket, Document, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Get_Bucket_CORS
           (Item.Catalog, Bucket, Document, Configured, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_CORS (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Put_Bucket_Encryption
           (Item.Catalog, Bucket, Document, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_Encryption (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_Lifecycle (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_Replication (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Put_Bucket_Website (Item.Catalog, Bucket, Document, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      if not Valid_Bucket_Name (Bucket) then
         ...
         Catalogs.Delete_Bucket_Website (Item.Catalog, Bucket, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
           Flyology.IO.Timeout_Error =>
   ...
   begin
      ...
      then
         ...
         Catalogs.Delete_Bucket_Analytics_Configuration
   ...
   begin
      ...
      then
         ...
         Catalogs.Delete_Bucket_Metrics_Configuration
           (Item.Catalog, Bucket, Identifier, Result);
      end if;
   exception
      when Flyology.Cancellation.Operation_Cancelled |
   ...
   begin
      ...
      Catalogs.Find_Multipart_Upload
        (Item.Catalog, Bucket, Key, Upload_ID, Upload_Options, Result);
      if Result /= Success then
         return;
      ...
      if Result /= Success then
         ...
         --  The catalog now owns Payload.  Clear the rollback flag before
         --  retiring the replaced immutable file: a cleanup failure must not
         --  delete the newly committed part.
         Published := False;
         if US.Length (Previous) > 0 then
            Delete_Payload_If_Present (Item, US.To_String (Previous));
         end if;
   ...
   begin
      ...
      then
         Result := Backend_Unavailable;
         return;
      elsif Upload_Options.Checksum.Method = Composite_Checksum then
         Prior := Multipart_Part_Number'First;
         First := True;
         for Reference of Parts loop
            if (First and then Reference.Number /= 1)
              or else
                (not First and then Reference.Number /= Prior + 1)
            then
               Result := Invalid_Part_Order;
               return;
            end if;
      ...
      for Index in Records.First_Index .. Records.Last_Index loop
         declare
            ...
            Reference : constant Multipart_Part_Reference :=
              Parts (Positive (Index - Records.First_Index + 1));
         begin
            if
              (Upload_Options.Checksum.Algorithm = No_Checksum
               and then Record_Value.Info.Checksum /=
                 No_Checksum_Information)
              or else
                (Upload_Options.Checksum.Algorithm /= No_Checksum
                 and then
                   (Record_Value.Info.Checksum.Algorithm /=
                      Upload_Options.Checksum.Algorithm
                    or else Record_Value.Info.Checksum.Method /=
                      Upload_Options.Checksum.Method
                    or else not Checksum_Engine.Valid_Digest
                      (US.To_String (Record_Value.Info.Checksum.Value),
                       Record_Value.Info.Checksum.Algorithm)))
            then
               Result := Backend_Unavailable;
               return;
              ...
              (Upload_Options.Checksum.Method = Composite_Checksum
               and then Reference.Checksum /= Record_Value.Info.Checksum)
              or else
            ...
            then
               Result := Invalid_Part;
               return;
            elsif Index /= Records.Last_Index
            ...
            Completed_Parts.Append
              (Completed_Object_Part'
                 (Number   => Reference.Number,
                  Size     => Record_Value.Info.Size,
                  Checksum => Record_Value.Info.Checksum));
            Include_Part_Digest
