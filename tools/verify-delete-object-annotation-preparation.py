#!/usr/bin/env python3
"""Verify DeleteObjectAnnotation inventory and corpus graph."""
...
CORPUS = ROOT / "tests/corpora/delete-object-annotation"
...
EXPECTED = [
    ("request", 167, 1, "Bucket", 60, "uri-label", "true", "projected"),
    ("request", 167, 2, "Key", 471, "uri-label", "true", "projected"),
    ("request", 167, 3, "AnnotationName", 37, "query", "true", "projected"),
    ("request", 167, 4, "VersionId", 492, "query", "false", "projected"),
    ("request", 167, 5, "RequestPayer", 599, "header", "false", "projected"),
    ("request", 167, 6, "ExpectedBucketOwner", 15, "header", "false", "projected"),
    ("request", 167, 7, "ObjectIfMatch", 470, "header", "false", "projected"),
    ("response", 166, 1, "ObjectVersionId", 492, "header", "false", "decoded"),
    ("response", 166, 2, "RequestCharged", 598, "header", "false", "decoded"),
]
...
def operation_scalar(model: str, function: str) -> str:
    match = re.search(
        r"when Delete_Object_Annotation_Operation =>\s+return\s+([^;]+);",
        function_body(model, function))
    if match is None:
        fail(f"DeleteObjectAnnotation lacks {function}")
...
def main() -> int:
    ...
    scalars = {
        ...
        "Request_URI": "/{Bucket}/{Key+}?annotation",
        "Response_Code": "204",
        "Input_Shape": "167",
        "Output_Shape": "166",
    ...
    if shape_scalar(model, "Kind", 167) != "Structure_Shape" or \
            shape_scalar(model, "Kind", 166) != "Structure_Shape":
        fail("generated operation structures changed kind")
    for shape in (15, 37, 470, 471, 492, 598, 599):
        if shape_scalar(model, "Kind", shape) != "String_Shape":
            fail(f"generated scalar shape {shape} changed kind")
    if shape_scalar(model, "Minimum", 471) != "1":
        fail("generated key minimum changed")
    for shape in (15, 37, 470, 492, 598, 599):
        if shape_scalar(model, "Minimum", shape) != "" or \
                shape_scalar(model, "Maximum", shape) != "" or \
                shape_scalar(model, "Pattern", shape) != "":
            fail(f"generated opaque scalar shape {shape} changed")
    if enum_values(model, 598) != ["requester"] or \
            enum_values(model, 599) != ["requester"]:
        fail("generated requester-pays enum changed")

    generated = []
    for direction, shape, boundary in (("request", 167, "projected"),
                                       ("response", 166, "decoded")):
    ...
    print("DeleteObjectAnnotation preparation: 9 modeled members, "
...
if __name__ == "__main__":
    try:
        ...
        print(f"DeleteObjectAnnotation verification failed: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
