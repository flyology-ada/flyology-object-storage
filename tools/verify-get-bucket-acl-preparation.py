#!/usr/bin/env python3
"""Verify GetBucketAcl model inventory and reciprocal corpus graph."""
...
CORPUS = ROOT / "tests/corpora/get-bucket-acl"
...
SOURCES = (
    ...
    ROOT / "src/flyology-object_storage-s3-xml.adb",
)
REVISION = "36c34f15391da01cd717c73c0fffa747c9889768"
SHA256 = "429763d64912af5edae4c7a0f20a8ac3e6fecf734cde5fc465016bc8badcdef9"
EXPECTED = [
    ("request", 226, 1, "Bucket", 60, "uri-label", "true", "projected"),
    ("request", 226, 2, "ExpectedBucketOwner", 15, "header", "false", "projected"),
    ("output", 225, 1, "Owner", 499, "body", "false", "decoded"),
    ("output", 225, 2, "Grants", 300, "body", "false", "decoded"),
...
def operation_scalar(model: str, function: str) -> str:
    match = re.search(r"when Get_Bucket_Acl_Operation =>\s+return\s+([^;]+);",
                      function_body(model, function))
    if match is None:
        fail(f"GetBucketAcl lacks {function}")
...
def main() -> int:
    ...
    scalars = {"Method": "Get_Method", "Request_URI": "/{Bucket}?acl",
               "Response_Code": "200", "Input_Shape": "226",
               "Output_Shape": "225", "Request_Checksum_Required": "False"}
    ...
    if enum_values(model, 514) != [
            "FULL_CONTROL", "WRITE", "WRITE_ACP", "READ", "READ_ACP"]:
        fail("generated permission enum changed")
    if enum_values(model, 696) != [
            "CanonicalUser", "AmazonCustomerByEmail", "Group"]:
        fail("generated grantee type enum changed")

    generated = []
    for direction, shape, boundary in (
            ("request", 226, "projected"), ("output", 225, "decoded"),
    ...
    attributes = case_values(model, "Member_XML_Attribute", 299)
    if attributes != ["False", "False", "False", "True", "False"]:
    ...
    for vector in vectors:
        ...
        if any(ref != "operation:GetBucketAcl" and ref not in member_keys
               for ref in refs):
            fail(f'{vector["id"]}: unknown member reference')
        if vector["id"] not in reached and "operation:GetBucketAcl" not in refs:
    ...
    print("GetBucketAcl preparation: 13 modeled members, one nonflattened "
          f"list, 8 exact enum values, {len(vectors)} vectors")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"GetBucketAcl verification failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
