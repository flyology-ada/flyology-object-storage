#!/usr/bin/env python3
"""Verify PutObjectLockConfiguration inventory and reciprocal coverage."""
...
CORPUS = ROOT / "tests/corpora/put-object-lock-configuration"
...
EXPECTED = [
    ("request", 552, 1, "Bucket", 60, "uri-label", "true", "projected"),
    ("request", 552, 2, "ObjectLockConfiguration", 473, "body", "false", "encoded"),
    ("request", 552, 3, "RequestPayer", 599, "header", "false", "projected"),
    ("request", 552, 4, "Token", 483, "header", "false", "projected"),
    ("request", 552, 5, "ContentMD5", 111, "header", "false", "projected"),
    ("request", 552, 6, "ChecksumAlgorithm", 77, "header", "false", "projected"),
    ("request", 552, 7, "ExpectedBucketOwner", 15, "header", "false", "projected"),
    ("nested", 473, 1, "ObjectLockEnabled", 474, "body", "false", "encoded"),
    ("nested", 473, 2, "Rule", 482, "body", "false", "encoded"),
    ("nested", 482, 1, "DefaultRetention", 143, "body", "false", "encoded"),
    ("nested", 143, 1, "Mode", 481, "body", "false", "encoded"),
    ("nested", 143, 2, "Days", 141, "body", "false", "encoded"),
    ("nested", 143, 3, "Years", 718, "body", "false", "encoded"),
    ("response", 551, 1, "RequestCharged", 598, "header", "false", "decoded"),
]
...
def operation_scalar(model: str, function: str) -> str:
    match = re.search(
        r"when Put_Object_Lock_Configuration_Operation =>\s+return\s+([^;]+);",
        function_body(model, function))
    if match is None:
        fail(f"PutObjectLockConfiguration lacks {function}")
...
def main() -> int:
    ...
    scalars = {
        "Method": "Put_Method", "Request_URI": "/{Bucket}?object-lock",
        "Response_Code": "200", "Input_Shape": "552",
        "Output_Shape": "551", "Request_Checksum_Required": "True",
    ...
    for shape in (552, 551, 473, 482, 143):
        if shape_scalar(model, "Kind", shape) != "Structure_Shape":
            fail(f"generated structure shape {shape} changed kind")
    if shape_scalar(model, "Kind", 141) != "Integer_Shape" or \
            shape_scalar(model, "Kind", 718) != "Integer_Shape":
        fail("generated Days or Years integer shape changed")
    if enum_values(model, 474) != ["Enabled"] or \
            enum_values(model, 481) != ["GOVERNANCE", "COMPLIANCE"]:
        fail("generated Object Lock configuration enum changed")
    ...
    if enum_values(model, 77) != checksums:
        fail("generated checksum enum changed")

    generated = []
    for direction, shape, boundary in (
            ("request", 552, "projected"), ("nested", 473, "encoded"),
            ("nested", 482, "encoded"), ("nested", 143, "encoded"),
            ("response", 551, "decoded")):
        ...
        for index, name in enumerate(names):
            member_boundary = "encoded" if shape == 552 and \
                name == "ObjectLockConfiguration" else boundary
    ...
    print("PutObjectLockConfiguration preparation: 14 modeled members, "
          f"10 exact checksum values, {len(vectors)} vectors")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"PutObjectLockConfiguration verification failed: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
