#!/usr/bin/env python3
"""Verify GetBucketOwnershipControls model inventory and corpus graph."""
...
CORPUS = ROOT / "tests/corpora/get-bucket-ownership-controls"
...
CONTROL_SPEC = ROOT / "src/flyology-object_storage-s3-bucket_controls.ads"
CONTROL_BODY = ROOT / "src/flyology-object_storage-s3-bucket_controls.adb"
...
EXPECTED = [
    ("request", 255, 1, "Bucket", 60, "uri-label", "true", "projected"),
    ("request", 255, 2, "ExpectedBucketOwner", 15, "header", "false", "projected"),
    ("output", 254, 1, "OwnershipControls", 501, "body", "false", "decoded"),
    ("nested", 501, 1, "Rules", 503, "body", "true", "decoded"),
    ("nested", 502, 1, "ObjectOwnership", 485, "body", "true", "decoded"),
]
MEMBER_HEADER = [
...
def operation_scalar(model: str, function: str) -> str:
    match = re.search(
        r"when Get_Bucket_Ownership_Controls_Operation =>\s+return\s+([^;]+);",
        function_body(model, function),
    )
    if match is None:
        fail(f"generated model lacks GetBucketOwnershipControls {function}")
...
def main() -> int:
    ...
    expected_scalars = {
        "Method": "Get_Method", "Request_URI": "/{Bucket}?ownershipControls",
        "Response_Code": "200", "Input_Shape": "255",
        "Output_Shape": "254", "Request_Checksum_Required": "False",
    }
    ...
    if re.search(
            r'when 254 =>\s+return "OwnershipControls";',
            function_body(model, "Payload_Member"),
    ) is None:
        fail("generated payload member changed")
    expected_counts = {255: 2, 254: 1, 501: 1, 502: 1}
    ...
    if shape_scalar(model, "Kind", 503) != "List_Shape" or \
            integer_for_shape(model, "List_Member_Shape", 503) != 502 or \
            shape_scalar(model, "Is_Flattened", 503) != "True" or \
            case_values(model, "Member_Location_Name", 501) != ["Rule"] or \
            case_values(model, "Member_Flattened", 501) != ["False"]:
        fail("generated flattened Rule list contract changed")
    ...
    for direction, shape, boundary in (
        ("request", 255, "projected"), ("output", 254, "decoded"),
        ("nested", 501, "decoded"), ("nested", 502, "decoded"),
    ):
    ...
    for vector in vectors:
        vector_id = vector["id"]
        if not re.fullmatch(r"GBO-(?:RQ|RS|TR)-\d{3}", vector_id):
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"GetBucketOwnershipControls verification failed: {exc}",
            file=sys.stderr,
        )
        raise SystemExit(1)
